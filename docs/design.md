...
A fourth finding came from the resources widget rather than the text. `Example-Deep-Dive/60-How-to-Debug-Chainguard-Images` has `description: "&mdash;Chainguard Containers Documentation"` inside a `<script>` element. Entities do not decode there, so the page shows the entity itself. The sibling copy is clean.
