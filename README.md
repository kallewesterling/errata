...
A fourth turned up while comparing the resources widget rather than the prose. `Example-Deep-Dive/60-How-to-Debug-Chainguard-Images` has `description: "&mdash;Chainguard Containers Documentation"` inside a `<script>`, where entities are not decoded, so it renders literally on the page. Its sibling copy is clean.
