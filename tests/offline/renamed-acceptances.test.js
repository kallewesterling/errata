...
describe("telling a retitled lesson apart from a repaired one", () => {
  ...
  it("recognizes a slug that changed case", () => {
    const { renamed, resolved } = partitionUnmatched(
      [entry("unpaired-output", "Course-A/60-Build-Your-First-Package/m7ssafa6bse3#7")],
      [finding("unpaired-output", "Course-A/60-Build-your-first-package/m7ssafa6bse3#7")],
    );
    expect(resolved).toEqual([]);
    expect(renamed).toHaveLength(1);
    expect(renamed[0].from).toBe("60-Build-Your-First-Package");
    expect(renamed[0].to).toBe("60-Build-your-first-package");
  });
